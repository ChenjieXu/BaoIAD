...
except ImportError:  # inventory/help paths do not require mmengine
    Config = None
...
# Repo-local benchmark scope: the 37 method entries maintained in
# baoiad/method_inventory.py. Load by file path so inventory/help paths stay
# usable without importing optional training dependencies from baoiad.__init__.
def _load_repo_method_slugs():
    path = os.path.join(ROOT, 'baoiad', 'method_inventory.py')
    spec = importlib.util.spec_from_file_location('baoiad_method_inventory', path)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    return module.method_slugs()


REPO_METHODS = _load_repo_method_slugs()
REPO_METHOD_SET = frozenset(REPO_METHODS)
...
_METHOD_CONFIG_PRIORITY = {
    ...
    'draem': [
        'draem_256_mvtec_strict.py',
    ],
    'dsr': [
        'dsr_256_mvtec_strict.py',
    ],
    'efficientad': [
        'efficientad_256_mvtec_strict.py',
    ],
    'fastflow': [
        'fastflow_wrn50_256_mvtec_strict.py',
    ],
    'ganomaly': [
        'ganomaly_256_mvtec_strict.py',
    ],
    'glass': [
    ...
    'nsa': [
        'nsa_rn18_256_mvtec_strict.py',
    ],
    'padim': [
        'padim_wrn50_256_mvtec_strict.py',
    ],
    'patchcore': [
        'patchcore_wrn50_256_mvtec_strict.py',
    ],
    'pyramidflow': [
    ...
    'vitad': [
        'vitad_256_mvtec_strict.py',
    ],
    'winclip': [
        'winclip_256_mvtec.py',
    ],
}

_DISABLED_STRICT_BENCHMARKS = {}
...
def disabled_strict_benchmark_reason(method, config_path):
    """Return a skip reason when a method's strict benchmark path is disabled."""
    if not method or not config_path:
        return None
    policy = _DISABLED_STRICT_BENCHMARKS.get(str(method).lower())
    if not policy:
        return None
    config_name = os.path.basename(config_path)
    if config_name in policy.get('strict_configs', set()):
        return policy.get('reason', 'strict benchmark is disabled')
    return None
...
def get_all_methods():
    """Return the repo-local 37-method inventory."""
    config_root = os.path.join(ROOT, 'configs')
    return [
        method for method in REPO_METHODS
        if os.path.isdir(os.path.join(config_root, method))
    ]


def is_iter_based(config_path):
    """Check if a config uses iteration-based training."""
    cfg = _load_config(config_path)
    if cfg is not None:
    ...
    content = _raw_config_text(config_path)
    return (
        'max_iters' in content
        or 'by_epoch=False' in content
        or 'IterBasedTrainLoop' in content
    )
...
def _should_use_direct_runner(config_path):
    """Use an in-process benchmark path for eval-only memory-bank methods."""
...
def main():
    ...
    for m in methods:
        if args.config:
            ...
            cfg = find_config(m)
        if cfg:
            disabled_reason = disabled_strict_benchmark_reason(m, cfg)
            if disabled_reason:
                print(f"[WARN] Skipping method '{m}': {disabled_reason}")
                continue
    ...
    for method, config_path in method_configs.items():
        ...
        for cat in method_categories:
            ...
            # Clear GPU memory between categories to prevent OOM accumulation
            import gc
            gc.collect()
            try:
