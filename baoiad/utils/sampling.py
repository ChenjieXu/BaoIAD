"""Sampling utilities for coreset selection.

This module contains sampling methods for selecting representative subsets
of features for anomaly detection.
"""
...
class kCenterGreedy:
    """Implements k-center-greedy method for coreset selection.
    ...
    def __init__(self, embedding: Tensor, sampling_size: int) -> None:
        ...
        if HAS_SKLEARN:
            self.model = SparseRandomProjection(n_components='auto', eps=0.9)
        else:
            self.model = None
    ...
    def select_coreset_idxs(self, selected_idxs: Optional[List[int]] = None) -> List[int]:
        ...
        if self.embedding.ndim == 2:
            if self.model is not None:
                self.model.fit(self.embedding.cpu().numpy())
                self.features = torch.Tensor(self.model.transform(self.embedding.cpu().numpy())).to(self.device)
            else:
                # Fallback: use original embedding without projection
                self.features = self.embedding.to(self.device)
