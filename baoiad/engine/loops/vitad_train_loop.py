"""Official-style ViTAD train loop."""
...
def vitad_official_collate(batch: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    """Collate ViTAD batches like the official PyTorch DataLoader."""
...
def _build_vitad_dataset(dataset_cfg: Dict[str, Any]):
...
def _build_vitad_official_dataloader(dataloader_cfg: Dict[str, Any]) -> DataLoader:
    dataloader_cfg = copy.deepcopy(dataloader_cfg)
    dataset = _build_vitad_dataset(dataloader_cfg.pop('dataset'))
    ...
    return DataLoader(
        ...
        collate_fn=vitad_official_collate,
    )
...
class ViTADOfficialTrainLoop(BaseLoop):
    ...
    MMEngine normally constructs the train loop and dataloader before the
    optimizer wrapper, while ADer effectively uses `model -> optimizer ->
    dataloader`. ViTAD is sensitive to that ordering because both the model
    construction and the dataset shuffle consume RNG. This loop stores the
    original dataloader config and only builds the raw PyTorch DataLoader when
    training actually starts, after `runner.optim_wrapper` already exists.
    """
    ...
    def dataloader(self):
        if self._dataloader is None:
            if isinstance(self._dataloader_cfg, dict):
                self._dataloader = _build_vitad_official_dataloader(self._dataloader_cfg)
            else:
    ...
    @property
    def max_iters(self):
        # Force lazy dataloader build before max_iters is queried.
        _ = self.dataloader
