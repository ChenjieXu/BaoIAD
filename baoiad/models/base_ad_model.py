...
class MemoryBankADModel(BaseADModel):
    """Base for memory-bank-based detectors (PatchCore, SPADE, PaDiM, DFKDE, DFM).
...
class KnowledgeDistillationADModel(BaseADModel):
    """Base for teacher-student detectors (RD, EfficientAD, RD++, STFPM, Dinomaly).
...
class FlowBasedADModel(BaseADModel):
    """Base for normalizing-flow detectors (FastFlow, CSFlow, CFlow, UFlow, DifferNet).
...
class ReconstructionADModel(BaseADModel):
    """Base for reconstruction-based detectors (DRAEM, MemSeg, DeSTSeg, MemAE).
