...
def _load_module():
    path = ROOT / 'tools' / 'cutpaste_density_transfer_diagnose.py'
    spec = importlib.util.spec_from_file_location('baoiad_cutpaste_density_transfer_diagnose', path)
...
def test_build_transfer_summary_reports_positive_gap_for_anomalies():
    ...
    summary = module._build_transfer_summary(labels, scores, train_embeddings, test_embeddings)
