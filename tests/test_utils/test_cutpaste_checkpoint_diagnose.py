...
def _load_module():
    path = ROOT / 'tools' / 'cutpaste_checkpoint_diagnose.py'
    spec = importlib.util.spec_from_file_location('baoiad_cutpaste_checkpoint_diagnose', path)
    ...
    ordered = [
        {
            ...
            'score_gap': {'score_gap_mean': 0.9, 'normal': {'mean': 0.1}, 'anomaly': {'mean': 1.0}},
        },
        {
            'label': 'iter_20',
            'metrics': {'image_auroc': 0.17, 'pixel_auroc': 0.41},
            'score_gap': {'score_gap_mean': -0.2, 'normal': {'mean': 0.8}, 'anomaly': {'mean': 0.6}},
        },
    ]
...
def test_build_transition_summary_reports_metric_deltas():
    ...
    assert transitions['iter_10 -> iter_20']['drift_stats_delta']['gde_fit_num_samples_delta'] == 2.0
    assert transitions['iter_10 -> iter_20']['drift_stats_delta']['test_anomaly_raw_l2_mean_delta'] == -0.7999999999999998
...
def test_build_compare_summary_keeps_source_label():
    ...
    assert compare['checkpoints']['iter_10']['drift_stats']['gde_fit_num_samples'] == 5.0
...
def test_build_cross_source_compare_reports_shared_label_deltas():
    ...
    assert summary['checkpoints']['iter_10']['metrics_delta']['image_auroc_delta'] == 0.019999999999999907
    assert summary['checkpoints']['iter_20']['score_gap_mean_delta'] == -0.8
    assert summary['checkpoints']['iter_10']['drift_stats_delta']['gde_fit_num_samples_delta'] == 1.0
    assert summary['checkpoints']['iter_20']['drift_stats_delta']['gde_mean_mean_delta'] == 0.3
...
def test_build_cross_source_compare_honors_reference_label_override():
    ...
    summary = module._build_cross_source_compare(current, reference, reference_source_label='manual_ref')
